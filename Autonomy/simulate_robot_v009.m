%	simulate_robot_v009.m
%
%   For FRC 2018 PowerUp
%   Modified for FRC 2018 Control by Adonis Canada, Priyanshu Agrawal,
%   and Baris Sengezen
%
%   v008    Dr K. 3/1/2019 Clean-up by 
%
%   v009    Dr K. 3/2/2019 Wall Alignment Control
...
Target.h            = 0.14; % [m] Game manual V9 page 30 5.5 in (approx 14 cm)  x  2 in (5 cm) angled at 14.5 deg
Target.w            = 0.05; % [m] Gamge manual target width 2 in, 5 cm

Target.d_top       = 0.20 + Target.w; % [m] Distance at the top (closest points on the targets, page 31
Target.d_bottom    = Target.d_top  + 2*cos(90*deg - 14.5*deg)*Target.h;     % [m] distance at the bottom of the targets
Target.d           = 1/2*(Target.d_top + Target.d_bottom);

%   NOTE! Target.x & Target.y are defined in Draw_Rocket_Line.m for FRC2019
%   !!!

N_pixel             = 160;%320;
Target.N_pixel      = N_pixel;  % store image width in pixel in Target structure for rectangle height calculations in units of pixels

camera_view         = 60*deg;   %   Camera HORIZONTAL viewing angle
Target.camera_view  = camera_view;
...
if make_movies,
    ...
    % Full field view for FRC2018
%     xlim([-6*ft Field.L + 5*ft])					% [m]	set figure limits for x-axis
%     ylim([-2*ft Field.W + 2*ft])					% [m]	set figure limits for y-axis

% ZOOMED for steering testing FRC2019
    xlim([4 8])					% [m]	set figure limits for x-axis
    ylim([-0.5 3])					% [m]	set figure limits for y-axis
...
while i<=N && ~at_target_flag,
    ...
    [h_pix_L, h_pix_R] = calcTargetHeights_v1(Target,Robot);
    ...
    %    t
        ...
        distance_ft_front_bumper = distance_ft - Robot.L/2/ft;
        
        if distance_ft_front_bumper < 0.1, % stop simulation at a small distance before target
            at_target_flag = 1;
        end
        
        [v_ft,omega] = Controller_v003(distance_ft_front_bumper, e_Gear_x, h_pix_L, h_pix_R, Robot);
